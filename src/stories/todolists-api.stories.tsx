...
const PostC = () => {
    ...
    return (
        <>
            ...
            <input placeholder={'enter todolist title'} value={value} onChange={(e) => setValue(e.currentTarget.value)}/>
...
const DeleteC = () => {
    ...
    const [value, setValue] = useState<string>('')
    useEffect(() => {
        todolistsAPI.getTodolists().then(res => setTodos(res.data.reduce((acc: Array<string>, el) => [...acc, el.id],[])))
    }, [state])
    ...
    return (
        <>
            ...
            <ul>{todos.map((el: any) => <li key={el}>{el}</li>)}</ul>
...
const UpdateC = () => {
    ...
    return (
        <>
            ...
            <input placeholder={'enter todolist title'} value={title} onChange={e => setTitle(e.currentTarget.value)}/>
